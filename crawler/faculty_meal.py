...
def clean_menu_item(item):
    item = item.strip()
    item = re.sub(r"^-+", "", item)
    item = item.replace("(통합)", "").strip()
    return item

def make_hash(date, time, menu):
    return hashlib.sha256((date + time + menu).encode("utf-8")).hexdigest()

def run_faculty_meal():
    print("🍽 한경국립대학교 교직원식당 식단표")
    print("=" * 30)

    url = "https://www.hknu.ac.kr/kor/671/subview.do"
    ...
    for table in tables:
        ...
        for row in rows:
            ...
            if current_date and time_cell and content_cell:
                ...
                cleaned_menu = [clean_menu_item(item) for item in menu_items if item.strip()]
                menu_text = "\n".join(cleaned_menu)
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT hash FROM faculty_meals")
    existing_hashes = {row[0] for row in cursor.fetchall()}

    sql = """
    INSERT INTO faculty_meals (meal_date, meal_time, menu, hash)
    SELECT %s, %s, %s, %s FROM DUAL
    WHERE NOT EXISTS (
        SELECT 1 FROM faculty_meals WHERE hash = %s
    )
    """
